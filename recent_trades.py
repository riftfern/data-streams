...
from symbols import TOP_100_LOWER, get_tier
...
if not os.path.isfile(trades_filename):
    with open(trades_filename, 'w') as f:
        f.write('Event Time,Symbol,Aggregate Trade ID,Price,Quantity,First Trade ID,is Buyer Maker,USD Size,Tier\n')
        ...
        while True:
            try:
                ...
                if usd_size > 14999:
                    ...
                    # Log to csv with tier info
                    with open(filename, 'a') as f:
                        f.write(f"{event_time},{symbol.upper()},{agg_trade_id},{price},{quantity},"
                            f"{trade_time},{is_buyer_maker},{usd_size},{tier}\n")
...
if __name__ == "__main__":
    try:
        print(f"Starting Binance Trade Stream for {len(symbols)} symbols...")
        asyncio.run(main())
